...
def perform_profile_interview(is_interview: bool) -> None:
    ...
    if is_interview:
        profile_metadata = pd.read_csv(
            f"data/{PROJECT}/profile_metadata_with_analysis.csv"
        )
    else:
        profile_metadata = pd.read_csv(
            f"data/{PROJECT}/keywordsearch_profile_metadata_test.csv"
        )
    video_metadata = pd.read_csv(
        f"data/{PROJECT}/profilesearch_video_metadata_test.csv"
    )
    video_metadata["createTime"] = pd.to_datetime(video_metadata["createTime"])
    ...
    if is_interview:  # Finfluencer interview
        ...
        profile_metadata["interview_system_message"] = profile_metadata.apply(
        ...
        profile_metadata["identification_system_message"] = profile_metadata.apply(
            construct_system_prompt, args=(is_interview,), axis=1
        )
    ...
    if is_interview:  # Finfluencer interview
        system_message_field = "interview_system_message"
        user_message_field = "interview_user_prompt"
        llm_response_field = "interview_llm_response"
    else:
        system_message_field = "identification_system_message"
        user_message_field = "identification_user_prompt"
        llm_response_field = "identification_llm_response"

    batch_file_dir = create_batch_file(
        profile_metadata,
        system_message_field=system_message_field,
        user_message_field=user_message_field,
    ...
    profile_metadata_with_responses.to_csv(
        f"data/{PROJECT}/profile_metadata_with_analysis.csv", index=False
    )
